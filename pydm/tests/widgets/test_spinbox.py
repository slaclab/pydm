...
def test_construct(qtbot):
    ...
    assert pydm_spinbox._limits_from_channel is True
    assert pydm_spinbox._user_lower_limit == 0.0
    assert pydm_spinbox._user_upper_limit == 100.0
...
def test_key_press_event(
        qtbot, signals, monkeypatch, first_key_pressed, second_key_pressed,
        keys_pressed_expected_results):
    """
    Test the widget's handling of the key press events.

    Expectations:
    The step exponent value of the spin box will change as the Left Arrow or
    ...
    first_key_pressed : Qt.Key
        The first key to press to change the spinbox's step exponent value
        (increase or decrease)
    second_key_pressed: Qt.Key
        The second key to press to change the spinbox's step exponent value
        (increase or decrease)
    keys_pressed_expected_results : tuple
        The new value after the first key, and then that after the second key,
        is pressed
    """
    pydm_spinbox = PyDMSpinbox()
    qtbot.addWidget(pydm_spinbox)
    ...
    def press_key_and_verify(
            key_pressed, key_mod, key_press_count, expected_exp,
            expected_value):
        if key_mod != Qt.NoModifier:
            # Monkeypatch the Control flag because in this test, we don't
            # properly have the app QApplication instance
            monkeypatch.setattr(
                QApplication, "queryKeyboardModifiers", lambda *args: key_mod)
        else:
            monkeypatch.setattr(
        ...
        for i in range(0, key_press_count):
            pydm_spinbox.keyPressEvent(QKeyEvent(
                        QEvent.KeyPress, key_pressed, key_mod))
        assert pydm_spinbox.step_exponent == expected_exp

        signals.send_value_signal[float].connect(signals.receiveValue)
        signals.send_value_signal[float].connect(
                                pydm_spinbox.channelValueChanged)

        # Send out the key event to update the value based on the step
        # exponent change
        pydm_spinbox.keyPressEvent(QKeyEvent(
    ...
    press_key_and_verify(
        first_key_pressed, Qt.ControlModifier, 2,
        keys_pressed_expected_results[0], INIT_SPINBOX_VALUE)

    # Send out the second key press event once to check if it can change the
    # step exponent
    press_key_and_verify(
        second_key_pressed, Qt.ControlModifier, 1,
        keys_pressed_expected_results[1], INIT_SPINBOX_VALUE)

    # Make sure the widget can process the UpArrow, DownArrow, PageUp, and
    # PageDown keys
    pydm_spinbox.keyPressEvent(QKeyEvent(
                QEvent.KeyPress, Qt.Key_Up, Qt.ControlModifier))
    pydm_spinbox.keyPressEvent(QKeyEvent(
                QEvent.KeyPress, Qt.Key_Down, Qt.ControlModifier))
    pydm_spinbox.keyPressEvent(QKeyEvent(
                QEvent.KeyPress, Qt.Key_PageUp, Qt.ControlModifier))
    pydm_spinbox.keyPressEvent(QKeyEvent(
                QEvent.KeyPress, Qt.Key_PageDown, Qt.ControlModifier))


def test_widget_ctx_menu(qtbot):
    """
    Test to make sure the widget's context menu contains the specific action
        for the widget.

    Expectations:
        All specific actions for the widget's context menu are present in that
        menu.
...
def test_update_step_size(qtbot, step_exp):
    ...
    Expectations:
    The widget's step exponent is incremented correctly, by verifying the
    value returned by the Qt API singleStep() is ten times the step exponent
    value.
...
def test_update_format_string(
        qtbot, signals, show_unit, new_unit, step_exp, show_step_exp):
    """
    Test the widget's capability of updating the format string when various
    unit and step exponent paramaters are updated. This method also test
    showStepExponent property and setter.
    ...
    show_step_exp : bool
        True if the step exponent is to be displayed on the widget; False if
        otherwise
    """
    pydm_spinbox = PyDMSpinbox()
    qtbot.addWidget(pydm_spinbox)
    ...
    if show_step_exp:
        ...
        expected_suffix = "{0} Step: 1E{1}".format(
                            units, pydm_spinbox.step_exponent)
...
def test_send_value(qtbot, signals, init_value, user_typed_value, precision):
    """
    Test sending the value from the widget to the channel. This method tests
        value_changed() and precision_changed().
    ...
    signals.new_value_signal[signal_type].connect(
                        pydm_spinbox.channelValueChanged)
    signals.new_value_signal[signal_type].emit(user_typed_value)

    # Besides receiving the new channel value, simulate the update of the new
    # value to the widget by connecting the channelValueChanged slot to the
    # same signal
    signals.send_value_signal[signal_type].connect(signals.receiveValue)
    signals.send_value_signal[signal_type].connect(
                                    pydm_spinbox.channelValueChanged)
...
@pytest.mark.parametrize("which_limit, chan_limit, user_limit", [
    ("UPPER", 123.456, 654.321),
    ("LOWER", 12.345, 54.321),
    ("UPPER", 123.456, 654.321),
    ("LOWER", 12.345, 54.321),
    ("UPPER", -12.3, -32.1),  # set maximum value smaller than current minimum
    ("LOWER", 123.45, 543.21),  # set minimum value larger than current maximum
])
def test_limit_setting(qtbot, signals, which_limit, chan_limit, user_limit):
    """
    Test the upper and lower limit settings.

    Expectations:
        The upper or lower limit can be set by emitting the channel signal and
        by user definition and subsequently read correctly.
    ...
    which_limit : str
        ...
        the lower limit
    chan_limit : float
        The new channel limit value
    user_limit : float
        The new user defined limit value
    """
    pydm_spinbox = PyDMSpinbox()
    qtbot.addWidget(pydm_spinbox)
    ...
    typ = type(chan_limit)
    if is_upper:
        sig = signals.upper_ctrl_limit_signal[typ]
        sig.connect(pydm_spinbox.upperCtrlLimitChanged)
        sig.emit(chan_limit)
        pydm_spinbox.userUpperLimit = user_limit
    else:
        sig = signals.lower_ctrl_limit_signal[typ]
        sig.connect(pydm_spinbox.lowerCtrlLimitChanged)
        sig.emit(chan_limit)
        pydm_spinbox.userLowerLimit = user_limit

    prec = pydm_spinbox.decimals()
    assert pydm_spinbox.limitsFromChannel is True
    if is_upper:
        assert pydm_spinbox.userUpperLimit == user_limit
        ...
        assert pydm_spinbox.userLowerLimit == user_limit
