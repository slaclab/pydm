...
class PyDMMultiStateLEDIndicator(QWidget, PyDMWidget):
    """
    widget should accept one or more binary or multi-bit binary PVs.
    There are 16 available states that can each be configured to a different color.
    ...
    def __init__(self, parent: Optional[QWidget] = None, init_channel=None):
        ...
        self._curr_state = 0
        self._enable_four_channel_input = False
    ...
    def value_changed(self, new_val: int) -> None:
        ...
        try:
            int(new_val)
            if new_val >= 0 and new_val <= 15:  # we use states 0-15 (16 total states)
