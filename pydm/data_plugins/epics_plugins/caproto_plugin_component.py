...
class Connection(PyDMConnection):
    def __init__(self, channel, pv, protocol=None, parent=None):
        ...
        conn = parse_channel_config(pv, force_dict=True)
        address = conn.get('parameters', {}).get('address')
        monitor_mask = SubscriptionType.DBE_VALUE
        monitor_mask |= SubscriptionType.DBE_ALARM
        monitor_mask |= SubscriptionType.DBE_PROPERTY

        self.pv = epics.get_pv(address, form='ctrl', auto_monitor=monitor_mask,
